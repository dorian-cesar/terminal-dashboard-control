...
function initializeData() {
    ...
    const defaultUsers = [
      {
        ...
        password: "admin123",
        role: "Administrador",
        sections: ["banos", "custodias", "parking", "andenes", "caja", "configuracion"],
      },
      {
        id: 2,
        email: "user@wit.la",
  ...
  if (!localStorage.getItem("vehicles")) {
    const defaultVehicles = [
      { patente: "ABCD12", empresaId: 2, destinoId: 1, horaIngreso: "08:30", estado: "activo" },
      { patente: "WXYZ98", empresaId: 3, destinoId: 4, horaIngreso: "10:15", estado: "activo" },
    ]
...
function addCompany(company) {
  const companies = getCompanies()
  company.id = companies.length > 0 ? Math.max(...companies.map((c) => c.id)) + 1 : 1
...
function addDestination(destination) {
  const destinations = getDestinations()
  destination.id = destinations.length > 0 ? Math.max(...destinations.map((d) => d.id)) + 1 : 1
