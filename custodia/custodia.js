...
(() => {
  ...
  async function completarProcesoEntrega(datos) {
    ...
    try {
      ...
      const updateRes = await callAPI(datosUpdate, urlUpdate);
      if (!updateRes || !updateRes.success) {
        throw new Error("Error actualizando registro de custodia");
      }

      // Preparar datos del comprobante PDF
      const datosTicket = {
      ...
      await enviarPdfAlServidor(pdf, "comprobante_entrega.pdf");

      alert("Entrega completada e impresión enviada correctamente.");
      btnLiberarImprimir.disabled = false;
      formulario.reset();
      document.getElementById("tabla-body").innerHTML = "";
    } catch (err) {
      console.error("Error completando proceso de entrega:", err);
      alert("Error al completar la entrega o imprimir el comprobante.");
      btnLiberarImprimir.disabled = false;
    }
  }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (typeof jspdf === "undefined" && typeof jsPDF === "undefined") {
          ...
          script.src =
            "https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js";
          script.onload = () => setTimeout(() => crearPDF(), 100);
          script.onerror = () => reject(new Error("No se pudo cargar jsPDF"));
          document.head.appendChild(script);
        } else {
          crearPDF();
        ...
        function crearPDF() {
          ...
          try {
            ...
            const imageData = canvas.toDataURL("image/png");
            pdf.addImage(imageData, "PNG", 3, y, 52, 16);
            y += 19;
            pdf.setFont("courier", "bold");
            pdf.setFontSize(10);
            pdf.text("CÓDIGO:", 3, y);
            ...
            pdf.setFont("courier", "bold");
            pdf.text(`CÓDIGO: ${datosTicket.codigoBarras}`, 3, y);
          }
