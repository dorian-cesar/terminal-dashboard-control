...
async function checkUserExist(pin) {
  try {
    ...
    const data = await response.json();
    // code 0 = existe, code -22 = no existe
    return data.code === 0;
  } catch (error) {
...
async function generarTokenNumericoUnico() {
  console.log("[ZKTeco] Buscando PIN único disponible...");

  while (true) {
    const pin = generarTokenNumerico();
    const existe = await checkUserExist(pin);

    if (!existe) {
      console.log(`[ZKTeco] PIN único encontrado y listo para usar: ${pin}`);
      return pin;
    }

    console.warn(`[ZKTeco] El PIN ${pin} ya está en uso, generando uno nuevo...`);
  }
}
